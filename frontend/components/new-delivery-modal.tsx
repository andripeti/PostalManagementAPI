...
import { useDeliveries, type DeliveryFormDto } from "@/lib/hooks/use-deliveries"
...
export default function NewDeliveryModal({ onClose }: { onClose: () => void }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      // Ensure all required numeric fields are properly converted
      const packageWeight = Number(values.package.weight)
      const packageLength = Number(values.package.dimensions.length)
      const packageWidth = Number(values.package.dimensions.width)
      const packageHeight = Number(values.package.dimensions.height)
      ...
      const deliveryData: DeliveryFormDto = {
        // Required sender fields
        senderName: values.sender.name,
        senderEmail: values.sender.email,
        senderPhone: values.sender.phone,
        // Optional sender address
        senderStreet: values.sender.address.street,
        senderCity: values.sender.address.city,
        senderState: values.sender.address.state,
        senderCountry: values.sender.address.country,
        senderZipCode: values.sender.address.postal_code,
        ...
        recipientStreet: values.recipient.address.street,
        recipientCity: values.recipient.address.city,
        recipientZipCode: values.recipient.address.postal_code,
        // Optional recipient fields
        recipientState: values.recipient.address.state,
        recipientCountry: values.recipient.address.country,

        // Required package fields
        packageType: values.package.type,
        packageWeight,
        // Optional package fields
        packageLength,
        packageWidth,
        packageHeight,
        notes: values.package.description,
      }
